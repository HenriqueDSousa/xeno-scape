...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
    ...
    // If no free channel, apply policy: stop the first non-looping sound.
    // If all are looping, stop channel 0.
    if (channel == -1) {
        int candidate = -1;
        for (int i = 0; i < static_cast<int>(mChannels.size()); ++i) {
            ...
            if (it != mHandleMap.end()) {
                ...
                // If handle not found in map, consider channel free
                candidate = i;
                break;
            }
        }
        ...
        // Stop that channel
        Mix_HaltChannel(candidate);
        // If there was a handle associated, remove it
        if (mChannels[candidate].IsValid()) {
            mHandleMap.erase(mChannels[candidate]);
            mChannels[candidate].Reset();
        }

        channel = candidate;
    }

    // Create a new handle and record info
    ++mLastHandle;
    ...
    int playRes = Mix_PlayChannel(channel, sound, loops);
    if (playRes == -1) {
        SDL_Log("[AudioSystem] Mix_PlayChannel failed for %s: %s", soundName.c_str(), Mix_GetError());
        // Cleanup mapping
...
// Pauses all sounds on all channels
void AudioSystem::PauseAllSounds()
{
    // Pause every channel
    Mix_Pause(-1);

    // Mark every active handle as paused
    for (auto& kv : mHandleMap)
    {
        kv.second.mIsPaused = true;
    }
}
...
{
    // Resume channels that we have marked as paused
    for (auto& kv : mHandleMap)
    {
        if (kv.second.mIsPaused)
        {
            Mix_Resume(kv.second.mChannel);
            kv.second.mIsPaused = false;
        }
    }
}
